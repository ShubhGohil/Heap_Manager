...
def error_entitybody(code):
	...
	if code==304:
		...
		response+="<p>Max number of connections limit reached.</p>\r\n"
...
def complete_error_response(clientsocket, code):
	...
	headers+="Server: Exite/1.6 Ubuntu\r\n"
	headers+=date+'\r\n'
...
#create response headers
	...
	for header in header_dict:
		if header=="host":
			...
			response_headlist.append("Server: Exite/1.6 Ubuntu")
...
def post_method(clientsocket, address, header_list, path, data=""):
			...
			for individual in all_data:
				if "Content-Disposition" in individual:
					...
					if 'filename' in individual:
						...
						log_data += "Location: " + DocumentRoot + '/postdata/' + filename
						flag = 1
					else:
						log_data += individual		
	...
	entity_headers+="Server: Exite/1.6 Ubuntu\r\n"
...
def client_request(clientsocket, address):
	...
	length = 0
	message = clientsocket.recv(byte) #accept the headers
